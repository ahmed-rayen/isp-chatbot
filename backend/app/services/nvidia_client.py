...
def get_ai_response_with_tools(db: Session, session_id: str, chat_history: list, transcript_str: str, user_id: str):
    ...
    if transcript_str:
        messages.append({
            "role": "system", 
            "content": f"If you decide to use the create_ticket tool, use the following text as the 'transcript' argument:\n{transcript_str}"
        })
    ...
    if user_id:
        messages.append({
            "role": "system", 
            "content": f"The authenticated user's ID is: {user_id}. Use this value when calling get_account_status or create_ticket."
        })
        ...
        for tool_call in message.tool_calls:
            ...
            if tool_name == "schedule_technician_visit":
                if not tool_args.get("preferred_date") or not tool_args.get("time_slot"):
                    ...
                    continue # Skip executing the tool, let the AI reply to the user instead
